#!/usr/bin/env python3
"""
AWS Security Group RDP Open Access Check
Identifies security groups that allow unrestricted inbound RDP access (port 3389)
from 0.0.0.0/0 or ::/0.
...
def main():
    ...
    try:
        ...
        for sg in security_groups:
            ...
            open_cidrs = is_port_open_to_world(sg, 3389)
            ...
            if open_cidrs:
                findings.append({
                    ...
                    "status": "FAIL",
                    "message": (
                        f"Security group '{sg_name}' ({sg_id}) allows RDP (port 3389) "
                ...
                findings.append({
                    ...
                    "resource_type": "AWS::EC2::SecurityGroup",
                    "status": "PASS",
                    "message": f"Security group '{sg_name}' ({sg_id}) does not allow unrestricted RDP access",
                    "details": json.dumps({
                        "group_id": sg_id,
                        "group_name": sg_name,
                        "vpc_id": vpc_id,
